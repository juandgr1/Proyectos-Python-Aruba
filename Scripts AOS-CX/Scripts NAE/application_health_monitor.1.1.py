...
from templateapi.logger import Logger
...
from templateapi.action import ActionSyslog, ActionCLI, ActionShell, ActionCustomReport
...
ParameterDefinitions = {
    ...
    'VoIP_IPSLA_Name': {
        'Name': '<Constant> VOIP IP-SLA session to monitor for anomalies',
        ...
        'ErrorMsg': "The VoIP_IPSLA_Name parameter value is invalid or was changed after agent creation. VoIP IP-SLA monitoring and analysis cannot operate without a valid IP-SLA name and this parameter value cannot be changed; configure an IP-SLA session with the original name or create a new agent using the correct value.",
        'Type': 'string',
        'Constant': 'True',
        'Default': ""
    },
    ...
    'HTTP_IPSLA_Name': {
        'Name': '<Constant> HTTP IP-SLA session to monitor for anomalies',
        ...
        'ErrorMsg': "The HTTP_IPSLA_Name parameter value is invalid or was changed after agent creation. HTTP IP-SLA monitoring and analysis cannot operate without a valid IP-SLA name and this parameter value cannot be changed; configure an IP-SLA session with the original name or create a new agent using the correct value.",
        'Type': 'string',
        'Constant': 'True',
        'Default': ""
    },
...
class TCP_SynAck:
    ...
    def __init__(self, agent):
        ...
        if self.ip.is_valid() and self.port.is_valid():
            ...
            syn_title = Title(
                "TCP Connection Set-up for Application Server {}, port {}".format(
                    self.ip.value(), self.port.value()))
            ...
            # Set up the web UI title and graph for SYN, SYN/ACK, and RST rates
            trf_title = Title(
                "Total TCP Traffic for Application Server {}, port {}".format(
                    self.ip.value(), self.port.value()))
    ...
    def valid_vrf(self, input):
        try:
            uri = "/rest/v1/system/vrfs/" + input
            r = get(HTTP_ADDRESS + uri, verify=False,
    ...
    def failed_ADC_detected(self, event):
        self.agent.report(
            ...
            title=Title("TCP Connection Monitoring Suspended"))
    ...
    def accepted_ADC_detected(self, event):
        self.agent.report("TCP connection monitoring is resumed.",
                          title=Title("TCP Connection Monitoring Resumed"))
    ...
    def syn_anomaly_detected(self, event):
        ...
        self.agent.report(content, title=Title(
            "TCP SYN-ACK/SYN Ratio Anomaly Detected"))
    ...
    def syn_normal_detected(self, event):
        ...
        self.agent.report(content, title=Title("TCP SYN Returned to Normal"))
...
class IPSLA:
    ...
    def __init__(self, ipsla_type, name_param, value_param, agent):
        ...
        self.ipsla_name = str(self.agent.params[name_param].value)
    ...
    def verify_ipsla_configuration(self):
        ...
        exists_uri = '/rest/v1/system/ipsla_sources/' + self.ipsla_name
        try:
            exists_response = get(
                HTTP_ADDRESS + exists_uri, verify=False,
                proxies={'http': None, 'https': None})
        ...
        state_monitor = self.agent.monitor(
            ...
            .format(self.ipsla_name), self.ipsla_type + '_' +
            self.ipsla_name + '_state_monitor')
        state_rule = self.agent.rule(
            ...
            self.ipsla_name +
            ' State Error',
            self.ipsla_state_callback,
            self.ipsla_state_clear_callback)
        ...
        type_monitor = self.agent.monitor(
            '/rest/v1/system/ipsla_sources/{}?attributes=type'.format(
                self.ipsla_name), self.ipsla_type + '_' + self.ipsla_name +
            '_type_monitor')
        type_rule = self.agent.rule(
            ...
            self.ipsla_name +
            ' Type Error',
            self.ipsla_type_callback,
            self.ipsla_type_clear_callback)
    ...
    def monitor_ipsla_status(self):
        ...
        ipsla_uri = '/rest/v1/system/ipsla_sources/{}?attributes='.format(
            self.ipsla_name)
        ...
        if self.ipsla_type == 'udp_jitter_voip':
            packets_rx_monitor = self.agent.monitor(
                ipsla_uri + 'sla_results.num_of_packets_rx', self.ipsla_type +
                '_' + self.ipsla_name + '_packets_rx_monitor')
            packets_rx_rule = self.agent.rule(
                'VOIP ' + self.ipsla_name + ' Packets RX Error',
                self.ipsla_status_callback,
                self.ipsla_status_clear_callback)
            packets_rx_rule.condition(
                '{} == 0', [packets_rx_monitor])
            packets_rx_rule.clear_condition(
                '{} > 0', [packets_rx_monitor])
        for stat in statistics:
            rate = Rate(ipsla_uri + 'statistics.' + stat, ipsla_rate_string)
    ...
    def create_ipsla_value_monitor(self):
        ...
        value_monitor = self.agent.monitor(
            '/rest/v1/system/ipsla_sources/{}'.format(self.ipsla_name) +
            '?attributes=sla_results.' + statistic, self.ipsla_type + '_' +
            self.ipsla_name + '_' + statistic + '_monitor')
        ...
        value_short_rule = self.agent.rule(
            ...
            monitor_desc +
            ' Anomaly',
            self.ipsla_value_anomaly_detected,
            self.ipsla_value_normal_detected)
        ...
        ipsla_value_title = Title(monitor_desc + ' Monitoring for IPSLA {}'
                                  .format(self.ipsla_name))
        self.agent.graph(
    ...
    def ipsla_state_callback(self, event):
        ...
        if test_name and rule_desc:
            ...
            exists_uri = '/rest/v1/system/ipsla_sources/' + test_name
            try:
                exists_response = get(
                    HTTP_ADDRESS + exists_uri, verify=False,
                    proxies={'http': None, 'https': None})
    ...
    def get_session(self):
        url = '/rest/v1/system/ipsla_sources/' + self.ipsla_name
        try:
            response = get(HTTP_ADDRESS + url)
...
class Agent(NAE):
    ...
    def on_agent_creation(self):
        ...
        # agent
        try:
            nae_agent = get(HTTP_ADDRESS + self.uri, verify=False,
    ...
    def report(self, content, title):
        ActionCustomReport(content, title=title)
    ...
    def callback_action(self, syslog_msg=[], cli_cmd=[], custom_rpt=[]):
        ...
        for msg in syslog_msg:
            # ActionSyslog(msg)
            custom_rpt += msg   # add these to the report, don't generate syslog messages
        for cmd in cli_cmd:
            ActionCLI(cmd)
    ...
    def report_invalid_parameters(self, event):
        ...
        for name, bad_value in self.parameter_errors.items():
            ...
            ActionCustomReport(p.error_msg.format(bad_value),
                               title=Title(p.name + " Parameter Error"))
            ...
            if name in self.parameter:
                ...
                if p.constant or p.validator(str(value['new'])) is False:
                    ...
                    ActionCustomReport(p.error_msg.format(
                        str(value['new'])), title=Title(p.name + " Parameter Error"))
                else:
